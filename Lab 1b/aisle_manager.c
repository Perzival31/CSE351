...
unsigned short get_section(unsigned long* aisle, int index) {
  ...
  unsigned short x = (*aisle>>(index*16)) & SECTION_MASK;
  return x;
}
...
unsigned short get_spaces(unsigned long* aisle, int index) {
  ...
  unsigned short x = get_section(aisle, index);
  unsigned y = x & SPACES_MASK;
  return y;
}
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
 ...
 unsigned short* addr = (unsigned short*) aisle + index;
    *addr = new_section;
}
