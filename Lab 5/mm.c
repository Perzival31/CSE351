...
void* mm_malloc(size_t size) {
  ...
  // Find a free block that has enough space
  ptr_free_block = search_free_list(req_size);

  //if there is not enough space in the heap request more and search again
  ...
  if(block_size - req_size >= MIN_BLOCK_SIZE){
    ...
    block_info* next_free = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    next_free -> size_and_tags = (block_size - req_size) | TAG_PRECEDING_USED;
    *((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, (block_size - WORD_SIZE))) = (block_size - req_size) | TAG_PRECEDING_USED;
    insert_free_block(next_free);
...
void mm_free(void* ptr) {
  ...
  //get the pointer to the following blokc in the heap
  following_block = (block_info*) UNSCALED_POINTER_ADD(block_to_free, payload_size);

  //clear preceding used tag
  ...
  //set boundry tag to the same size_and_tags field that is in the header
